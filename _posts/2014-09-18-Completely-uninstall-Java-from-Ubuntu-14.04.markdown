...
<div class="row">	
	<div class="span9 columns">
		<p>There maybe many reasons why you might want to remove Java from your system. Whatever the reasons, follow the procedure below to uninstall Java from your Ubuntu system:</p>
		<h2>Remove all the Java related packages</h2>
		<p>First run the below three commands one by one to remove all Java related packages like <b>Sun, Oracle, OpenJDK, IcedTea plugins, GIJ</b>:</p>
		<script src="https://gist.github.com/ajgupta/44443aa08c058e4255d8.js"></script>
		<h2>Purge config files</h2>
		<p>After removing all the packages you need to purge all the <b>config files</b>. For that run the following command:</p>
		<script src="https://gist.github.com/ajgupta/2a0438a3088848d9939e.js"></script>
		<h2>Remove cache directory</h2>
		<p>After purging the config files we need to remove Java config and cache directory from your system. Run the following command to remove the cache directory:</p>
		...
		<p>After uninstalling all the java components from your system some java entries might still be left in your system. To get rid of them run the following command:</p>
